...
from lib.net.Transformer import FaceRoI
...
from common.nets.module_wo_face_decoder import PositionNet,BoxNet
...
class HGPIFuNet(BasePIFuNet):
    ...
    def filter(self, in_tensor_dict, return_inter=False):
        ...
        """
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 确保使用正确的设备

         # 确保输入张量也在这个设备上
        image = in_tensor_dict["image"].to(device)
        ...
        # 初始化 ViTEncoder 并移动到 GPU
        self.encoder = ViTEncoder(image_size=512, patch_size=16, dim=256, depth=8, heads=8, mlp_dim=2048).to(device)
        img_feat = self.encoder(image.to(device))  # 确保输入图像也在 GPU 上

        # 初始化 PositionNet 并移动到 GPU
        self.body_position_net = PositionNet('body', feat_dim=cfg.feat_dim).to(device)
        body_joint_hm, body_joint_img = self.body_position_net(img_feat)

        # 初始化 BoxNet 并移动到 GPU
        self.box_net = BoxNet(feat_dim=cfg.feat_dim).to(device)
        ...
        if self.prior_type in ["icon", "keypoint"]:
            if self.use_filter:
                print("调试img_feat="+str(img_feat)+"调试face_bbox="+str(face_bbox)+"调试smpl_normals="+str(smpl_normals))
                triplane_features = self.image_filter(face_bbox,fuse_image, smpl_normals)
    ...
    def query(self, features, points, calibs, transforms=None, type='shape'):
        ...
        if len(features) == 5:
            ...
            final_feat = torch.cat([triplane_feat, bary_centric_feat.permute(0, 2, 1), normal_feat],
                                   dim=1)  # 64+64+6=134
